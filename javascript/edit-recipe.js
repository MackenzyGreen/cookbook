//jquery api pull to get the data and then fill it in.
//will need to alter the two counts according to existing steps/ingredients.
...
$(document).ready(function(){
    ...
    deleteBtns.forEach(element => {
        $("#"+element.id).click(deleteSlot);
    });

    $("#"+addBtn[0].id).click(addSlot);

    //directions buttons
    container = document.querySelector(".direction-list");
    deleteBtns = container.querySelectorAll(" div > input[id ^='directionDelete']");
    addBtn = container.querySelectorAll("div > input[id ^='directionAdd']");

    deleteBtns.forEach(element => {
        $("#"+element.id).click(directionDeleteSlot);
    });

    $("#"+addBtn[0].id).click(directionAddSlot);
});
